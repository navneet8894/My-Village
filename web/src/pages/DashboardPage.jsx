...
export default function DashboardPage() {
  ...
  if (user?.role === 'admin') {
    return (
      <div>
        ...
        <p className="text-text-subtle mt-1">Admin — use the sidebar to manage villages and users.</p>
        <Link
          to="/dashboard/admin"
          className="inline-block mt-6 rounded-lg bg-primary text-primary-contrast px-5 py-2.5 font-medium"
        >
  ...
  return (
    <div>
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        {!hasVillage && (
          <button
            ...
            className="rounded-lg bg-primary hover:bg-primary-hover text-primary-contrast px-4 py-2 text-sm font-medium shadow-sm"
          >
            Add your village
          </button>
        )}
      ...
      {!hasVillage && (
        <div className="mt-6 rounded-xl border-2 border-dashed border-primary bg-primary-muted p-6 text-center">
          ...
          <button
            type="button"
            onClick={() => setModalOpen(true)}
            className="mt-4 rounded-lg bg-primary text-primary-contrast px-5 py-2 font-medium"
          >
            Add your village
          </button>
        </div>
      ...
      {hasVillage && village && (
        <div className="mt-6 rounded-xl border border-line bg-card p-5">
          ...
          <p className="text-primary-text font-medium mt-1">{village.name}</p>
          ...
          {members.length === 0 ? (
            <p className="text-sm text-text-subtle">You are the first member in this village.</p>
          ) : (
            <ul className="grid sm:grid-cols-2 gap-3">
              {members.map((m) => (
                ...
                >
                  <div className="h-10 w-10 rounded-full bg-primary-soft flex items-center justify-center font-semibold text-primary-text">
      ...
      <div className="mt-8 grid sm:grid-cols-2 gap-4">
        {cards.map((c) => (
          <Link
            key={c.to}
            to={c.to}
            className="rounded-xl border border-line p-5 hover:border-primary hover:shadow-md transition bg-card"
          >
            <div className="font-semibold text-lg">{c.title}</div>
            <p className="text-sm text-text-subtle mt-1">{c.desc}</p>
          </Link>
        ))}
      </div>
